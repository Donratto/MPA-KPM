...
main(int argc, char* argv[])
{
    ...
    double centralFrequencyBand1 = 2050e6;
    double bandwidthBand1 = 50e6;
    double centralFrequencyBand2 = 2200e6;
    double bandwidthBand2 = 100e6;

    double bandwidth = 20e6;

    uint16_t numerologyBwp0 = 1;
    uint16_t numerologyBwp1 = 1;//2 3
    uint16_t numerologyBwp2 = 1;
    uint16_t numerologyBwpDl = 1;
    uint16_t numerologyBwpUl = 1;
    ...
    ueNodes2.Create(ueNumPergNb2);


    Ptr<ListPositionAllocator> apPositionAlloc = CreateObject<ListPositionAllocator>();
    Ptr<ListPositionAllocator> staPositionAlloc = CreateObject<ListPositionAllocator>();
    Ptr<ListPositionAllocator> staPositionAlloc2 = CreateObject<ListPositionAllocator>();
    int32_t yValue = 0.0;
    ...
    for (uint32_t i = 1; i <= gNbNodes.GetN(); ++i)
    {
        ...
        double xValue2 = 0.0;
        for (uint32_t j = 1; j <= ueNumPergNb2; ++j)
        {
            if (j % 2 != 0)
            {
                xValue2 = j;
            ...
            if (yValue > 0)
            {
                staPositionAlloc2->Add(Vector(xValue2, 10, ueHeight));
            }
            else
            {
                staPositionAlloc2->Add(Vector(xValue2, -10, ueHeight));
    ...
    mobility.SetPositionAllocator(staPositionAlloc2);
    mobility.Install(ueNodes2);

    Ptr<NrPointToPointEpcHelper> epcHelper = CreateObject<NrPointToPointEpcHelper>();
    Ptr<IdealBeamformingHelper> idealBeamformingHelper = CreateObject<IdealBeamformingHelper>();
    Ptr<NrHelper> nrHelper = CreateObject<NrHelper>();
    Ptr<NrHelper> nrHelper2 = CreateObject<NrHelper>();
    ...
    nrHelper2->SetBeamformingHelper(idealBeamformingHelper);
    nrHelper2->SetEpcHelper(epcHelper);

    nrHelper2->SetPathlossAttribute("ShadowingEnabled", BooleanValue(false));
    ...
    nrHelper2->SetSchedulerAttribute("FixedMcsDl", BooleanValue(false));
    nrHelper2->SetSchedulerAttribute("FixedMcsUl", BooleanValue(false));

    // Error Model: UE and GNB with same spectrum error model.
    nrHelper2->SetUlErrorModel(errorModel);
    nrHelper2->SetDlErrorModel(errorModel);

    // Both DL and UL AMC will have the same model behind.
    nrHelper2->SetGnbDlAmcAttribute(
        "AmcModel",
        EnumValue(NrAmc::ErrorModel)); // NrAmc::ShannonModel or NrAmc::ErrorModel
    nrHelper2->SetGnbUlAmcAttribute(
        "AmcModel",
        EnumValue(NrAmc::ErrorModel)); // NrAmc::ShannonModel or NrAmc::ErrorModel
     ...
     * which is larger than in NR. We assume a value of 4 (could be 3 too).
     */
    nrHelper2->SetGnbDlAmcAttribute("NumRefScPerRb", UintegerValue(2));
    nrHelper2->SetGnbUlAmcAttribute("NumRefScPerRb", UintegerValue(2));
    nrHelper2->SetGnbMacAttribute("NumRbPerRbg", UintegerValue(4));
    nrHelper2->SetSchedulerAttribute("DlCtrlSymbols", UintegerValue(1));
    nrHelper2->SetSchedulerTypeId(TypeId::LookupByName("ns3::NrMacSchedulerOfdmaPF"));
...
BandwidthPartInfoPtrVector allBwps2;
    ...
    band1.m_centralFrequency = centralFrequencyBand1;
    band1.m_channelBandwidth = bandwidthBand1;
    band1.m_lowerFrequency = band1.m_centralFrequency - band1.m_channelBandwidth / 2;
    band1.m_higherFrequency = band1.m_centralFrequency + band1.m_channelBandwidth / 2;

    // Component Carrier 0
    std::unique_ptr<ComponentCarrierInfo> cc10(new ComponentCarrierInfo());
    cc10->m_ccId = 0;
    cc10->m_centralFrequency = band1.m_lowerFrequency + bandwidth;
    cc10->m_channelBandwidth = bandwidth;
    cc10->m_lowerFrequency = cc10->m_centralFrequency - cc10->m_channelBandwidth / 2;
    cc10->m_higherFrequency = cc10->m_centralFrequency + cc10->m_channelBandwidth / 2;

    // BWP 0
    std::unique_ptr<BandwidthPartInfo> bwp10(new BandwidthPartInfo());
    bwp10->m_bwpId = 0;
    bwp10->m_centralFrequency = cc10->m_centralFrequency;
    bwp10->m_channelBandwidth = cc10->m_channelBandwidth;
    bwp10->m_lowerFrequency = cc10->m_lowerFrequency;
    bwp10->m_higherFrequency = cc10->m_higherFrequency;

    cc10->AddBwp(std::move(bwp10));

    band1.AddCc(std::move(cc10));

    // Create the configuration for band38
    OperationBandInfo band3;
    band3.m_centralFrequency = centralFrequencyBand2;
    band3.m_channelBandwidth = bandwidthBand2;
    band3.m_lowerFrequency = band3.m_centralFrequency - band3.m_channelBandwidth / 2;
    band3.m_higherFrequency = band3.m_centralFrequency + band3.m_channelBandwidth / 2;

    //(CC1 - BWP1)
    // Component Carrier 1
    std::unique_ptr<ComponentCarrierInfo> cc11(new ComponentCarrierInfo());
    cc11->m_ccId = 1;
    cc11->m_centralFrequency = band3.m_lowerFrequency + bandwidth;
    cc11->m_channelBandwidth = bandwidth;
    cc11->m_lowerFrequency = cc11->m_centralFrequency - cc11->m_channelBandwidth / 2;
    cc11->m_higherFrequency = cc11->m_centralFrequency + cc11->m_channelBandwidth / 2;

    // BWP 1
    std::unique_ptr<BandwidthPartInfo> bwp12(new BandwidthPartInfo());
    bwp12->m_bwpId = 1;
    bwp12->m_centralFrequency = cc11->m_centralFrequency;
    bwp12->m_channelBandwidth = cc11->m_channelBandwidth;
    bwp12->m_lowerFrequency = cc11->m_lowerFrequency;
    bwp12->m_higherFrequency = cc11->m_higherFrequency;

    cc11->AddBwp(std::move(bwp12));

    std::unique_ptr<ComponentCarrierInfo> cc12(new ComponentCarrierInfo());
    std::unique_ptr<BandwidthPartInfo> bwp13(new BandwidthPartInfo());
    std::unique_ptr<BandwidthPartInfo> bwpdl2(new BandwidthPartInfo());
    std::unique_ptr<BandwidthPartInfo> bwpul2(new BandwidthPartInfo());

    // Component Carrier 2
    cc12->m_ccId = 2;
    cc12->m_centralFrequency = band3.m_higherFrequency - bandwidth;
    cc12->m_channelBandwidth = bandwidth;
    cc12->m_lowerFrequency = cc12->m_centralFrequency - cc12->m_channelBandwidth / 2;
    cc12->m_higherFrequency = cc12->m_centralFrequency + cc12->m_channelBandwidth / 2;

    if (operationMode == "TDD") //(CC2 - BWP2)
    {
        bwp13->m_bwpId = 1;
        bwp13->m_centralFrequency = cc12->m_centralFrequency;
        bwp13->m_channelBandwidth = cc12->m_channelBandwidth;
        bwp13->m_lowerFrequency = cc12->m_lowerFrequency;
        bwp13->m_higherFrequency = cc12->m_higherFrequency;

        cc12->AddBwp(std::move(bwp12));
    }
    else // FDD case  (CC2 - BWPdl & BWPul)
    {
        // BWP DL
        bwpdl2->m_bwpId = 2;
        bwpdl2->m_channelBandwidth = cc12->m_channelBandwidth / 2;
        bwpdl2->m_lowerFrequency = cc12->m_lowerFrequency;
        bwpdl2->m_higherFrequency = bwpdl2->m_lowerFrequency + bwpdl2->m_channelBandwidth;
        bwpdl2->m_centralFrequency = bwpdl2->m_lowerFrequency + bwpdl2->m_channelBandwidth / 2;

        cc12->AddBwp(std::move(bwpdl));

        // BWP UL
        bwpul2->m_bwpId = 3;
        bwpul2->m_channelBandwidth = cc12->m_channelBandwidth / 2;
        bwpul2->m_lowerFrequency = cc12->m_centralFrequency;
        bwpul2->m_higherFrequency = cc12->m_higherFrequency;
        bwpul2->m_centralFrequency = bwpul2->m_lowerFrequency + bwpul2->m_channelBandwidth / 2;

        cc12->AddBwp(std::move(bwpul2));
    }

    band3.AddCc(std::move(cc11));
    band3.AddCc(std::move(cc12));
    ...
    nrHelper2->InitializeOperationBand(&band1);
    nrHelper2->InitializeOperationBand(&band3);
    ...
    nrHelper->SetGnbAntennaAttribute("AntennaElement",
        ...
        // Antennas for all the UEs
    nrHelper2->SetUeAntennaAttribute("NumRows", UintegerValue(1));
    nrHelper2->SetUeAntennaAttribute("NumColumns", UintegerValue(1));
    nrHelper2->SetUeAntennaAttribute("AntennaElement",
                                    PointerValue(CreateObject<IsotropicAntennaModel>()));
    ...
    nrHelper->SetUeBwpManagerAlgorithmAttribute("NGBR_VOICE_VIDEO_GAMING",
        ...
        nrHelper2->SetGnbBwpManagerAlgorithmAttribute("NGBR_LOW_LAT_EMBB",
                                                 UintegerValue(bwpIdForLowLat));
    nrHelper2->SetGnbBwpManagerAlgorithmAttribute("GBR_CONV_VOICE", UintegerValue(bwpIdForVoice));
    nrHelper2->SetGnbBwpManagerAlgorithmAttribute("NGBR_VIDEO_TCP_PREMIUM",
                                                 UintegerValue(bwpIdForVideo));
    nrHelper2->SetGnbBwpManagerAlgorithmAttribute("NGBR_VOICE_VIDEO_GAMING",
                                                 UintegerValue(bwpIdForVideoGaming));

    nrHelper2->SetUeBwpManagerAlgorithmAttribute("NGBR_LOW_LAT_EMBB", UintegerValue(bwpIdForLowLat));
    nrHelper2->SetUeBwpManagerAlgorithmAttribute("GBR_CONV_VOICE", UintegerValue(bwpIdForVoice));
    nrHelper2->SetUeBwpManagerAlgorithmAttribute("NGBR_VIDEO_TCP_PREMIUM",
                                                UintegerValue(bwpIdForVideo));
    nrHelper2->SetUeBwpManagerAlgorithmAttribute("NGBR_VOICE_VIDEO_GAMING",
                                                UintegerValue(bwpIdForVideoGaming));                                            
    ...
    //NetDeviceContainer ueNetDev2 = nrHelper2->InstallUeDevice(ueNodes2, allBwps);
    ...
    // Set the default gateway for the UEs
    for (uint32_t j = 0; j < ueNodes2.GetN(); ++j)
    {
        Ptr<Ipv4StaticRouting> ueStaticRouting2 =
            ipv4RoutingHelper.GetStaticRouting(ueNodes2.Get(j)->GetObject<Ipv4>());
